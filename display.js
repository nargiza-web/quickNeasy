  ...
  data.forEach(doc => {
    ...
    const li = `
      <li style="width:175px">
        <div><input  onclick="showpic('${list.image}','${list.ingredients}','${list.name}','${list.preparation}',${list.servings},'${list.url}')" style="margin:25px 10px 3px 0px;float:left;width:100px; border-radius: 5px; border: 3px solid green lighten-1;" type="image" src="${list.image}" onerror="this.onerror=null;this.src='https://via.placeholder.com/200';" ><span style="clear:left;display:block;text-align:left;padding:5px 15px 5px 0px">${list.name}</span></div>
      </li>
    `;
    html += li;
  });
